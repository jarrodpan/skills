...
Treat generated output as non-source during search, audits, and refactors unless the task explicitly targets build artifacts or export output.

Ignore these paths by default:

- `src/Godot/.godot/`
- any `bin/`
- any `obj/`

When scanning file sizes, searching for seams, or choosing files to edit, filter those paths out first so generated output does not distort the repo picture.
...
After each change, generate a commit message describing the changes in brief.
